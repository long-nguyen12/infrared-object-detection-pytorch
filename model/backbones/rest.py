...
from model.modules.conv_layers import DropPath
...
class Attention(nn.Module):
    def __init__(self, dim, head, sr_ratio=1):
        ...
        if sr_ratio > 1:
            self.sr = nn.Conv2d(
                dim, dim, sr_ratio + 1, sr_ratio, sr_ratio // 2, groups=dim
            )
...
rest_settings = {
    "S": [[64, 128, 256, 512], [1, 1, 1, 1], 0.1],  # [embed_dims, depths, dpr]
...
class ResT(nn.Module):
    def __init__(self, model_name: str = "S") -> None:
        super().__init__()
        assert (
            model_name in rest_settings.keys()
        ), f"ResT model name should be in {list(rest_settings.keys())}"
        ...
        self.stage3 = nn.ModuleList(
            [Block(embed_dims[2], 4, 2, dpr[cur + i]) for i in range(depths[2])]
        )
        ...
        self.cbam_0 = CBAM(self.channels[0])
        self.cbam_1 = CBAM(self.channels[1])
        self.cbam_2 = CBAM(self.channels[2])
        self.cbam_3 = CBAM(self.channels[3])
